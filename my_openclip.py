...
class OpenCLIPNetwork:
    def __init__(self, device, negatives=None):
        ...
        # self.negatives = ("wall", "background")
    ...
    def get_relevancy(self, embed: torch.Tensor, positive_id: int) -> torch.Tensor:
        # embed: [h*w, 512] self.pos_embeds: [k, 512] self.neg_embeds: [4, 512]
        phrases_embeds = torch.cat([self.pos_embeds, self.neg_embeds], dim=0)
        p = phrases_embeds.to(embed.dtype) # [k + 4, 512]
        output = torch.mm(embed, p.T) # [h*w, k + 4]
        ...
        best_id = softmax[..., 0].argmin(dim=1) # [h*w,]: id of the most related in ("object", "things", "stuff", "texture")
    ...
    def get_max_across(self, sem_map):
        ...
        clip_output = sem_map.permute(1, 2, 0, 3).flatten(0, 1) # [h*w, nl, 512]

        n_levels_sims = [None for _ in range(n_levels)]
        for i in range(n_levels):
            for j in range(n_phrases):
                probs = self.get_relevancy(clip_output[..., i, :], j) # [h*w, 2]
                pos_prob = probs[..., 0:1] # take positive part only
